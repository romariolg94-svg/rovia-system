...
function MoreServices() { return <section id="servicii" className="bg-[#020b18] py-24"><SectionTitle small="Servicii extinse" title="Software, infrastructură și securitate într-o singură echipă" text="Rovia System acoperă tot traseul: analiză, dezvoltare, implementare, echipamente, mentenanță și suport." /><div className="mx-auto mt-14 grid max-w-[1400px] grid-cols-1 gap-6 px-5 md:grid-cols-2 md:px-10 lg:grid-cols-3">{extraServices.map(([Icon, title, text]) => <div key={title as string} className="rounded-3xl border border-white/10 bg-white/[0.04] p-8 hover:border-cyan-400/40 hover:bg-cyan-400/[0.06]"><Icon className="mb-6 text-cyan-300" size={38} /><h3 className="text-xl font-black">{title}</h3><p className="mt-4 leading-7 text-slate-300">{text}</p></div>)}</div></section>; }
